...
FinishedLabel=安裝程式已在您的電腦上安裝了 [name]。您可以透過已建立的捷徑啟動該應用程式。
FinishedRestartLabel=要完成 [name] 的安裝，需要重新啟動電腦。是否現在重新啟動？
ExitSetupTitle=結束安裝
ExitSetupMessage=安裝尚未完成。如果現在結束，程式將不會被安裝。%n%n您可以稍後再次執行安裝程式來完成安裝。%n%n結束安裝？

[CustomMessages]
CreateDesktopIcon=建立桌面捷徑
CreateStartMenuIcon=在開始功能表中建立捷徑
AdditionIcons=圖示設定: